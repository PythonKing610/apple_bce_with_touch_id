...
static void bce_completion_work_handler(struct work_struct *work)
{
    struct apple_bce_device *bce = container_of(work, struct apple_bce_device, completion_work);
    int i;
...
{
    struct apple_bce_device *bce = pci_get_drvdata(dev);
    schedule_work(&bce->completion_work);
    return IRQ_HANDLED;
}
